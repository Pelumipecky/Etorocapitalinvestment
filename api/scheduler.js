...
async function sendInvestmentCompletionEmail(userEmail, userName, investmentPlan, totalROI, bonusAmount, currentBalance) {
  ...
  try {
    await emailService.sendInvestmentCompleted(userEmail, userName, investmentPlan, totalROI, bonusAmount, currentBalance);
    console.log(`✅ Investment completion email sent to ${userEmail}`);
  } catch (error) {
    console.error(`❌ Failed to send investment completion email to ${userEmail}:`, error);
  }
}
